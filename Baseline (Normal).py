class Equipartition:
    ...
    def Jac_AP(poligono, number_of_regions,external, intermediate, internal, delta):
        ...
        ct = Equipartition.cumulative_jacap_times
        ct['AP_initial']       += t_ap
    ...
    def partition(polygon,number_of_regions,trial_id,round,order,seed):
        ...
        eqq = ['central_diff','factor','muti_fuzz','svr_fuzz','elas_fuzz','nn_pred_fuzz','normal']
        ...
        while (alcanzo_resultado==False) and (repetition < numero_de_repetitiones):
            ...
            while (t < iteraciones_Newton) and ((error_total_normalizado >10**-16)):
                ...
                filled_len = int(percent / 100 * bar_length)
                bar = '0' * filled_len + '-' * (bar_length - filled_len)
                print(f"\r Tr : {order} eqq : {eqq[6]} Iterations Normal Flow : |{bar}| {percent:.1f}%", end='', flush=True)
            ...
            save_csv = (f'TR : {trial_id}_{round} Total_Time: {endtime} CPU_time : {end_cputime}\n',
                  ...
                  ,Equipartition.get_jacap_timings())
            Equipartition.save_regions_to_csv(parti[0],os.path.join('k99', f'regions_coordinates_{trial_id}_{round}.csv'),trial_id,
                                    error_total_normalizado,order ,polygon ,sigmaa ,repetition ,save_csv ,number_of_regions ,t1 ,t
                                      ,endlloyd ,endnormal ,eqq[6] ,endtime)
        pass_key = 1
        ...
        if repetition == 1:
          ...
          for i in range(2,6):
            Equipartition.save_regions_to_csv(parti[0],os.path.join('k99', f'regions_coordinates_{trial_id}_{round}.csv'),trial_id,
                                  error_total_normalizado,order ,polygon ,sigmaa ,repetition ,save_csv ,number_of_regions ,t1 ,t
                                    ,endlloyd ,endnormal ,eqq[i] ,endtime)
            print("Returning pass_key =", pass_key)
        return parti[0],parti[1],parti[2],pass_key
