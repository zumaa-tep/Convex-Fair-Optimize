class svr_fuzz_Equipartition:
    ...
    def puntos_en_region_n(puntos_iniciales, points,j):
        ...
        for i in range(0,np.shape(points)[0]):
            if j in svr_fuzz_Equipartition.puntos_mas_cercanos(puntos_iniciales, points[i]):
                v.append(i)
        return(v)
    ...
    def centroide(R):
        ...
        areatotal= svr_fuzz_Equipartition.area(R)
    ...
    def triple_intersect(pt1,pt2,pt3):
        return svr_fuzz_Equipartition.vect_intersect(svr_fuzz_Equipartition.weighted_ridge(pt1,pt2),svr_fuzz_Equipartition.weighted_ridge(pt2,pt3))
        ...
        for i in range (0,length_sitios-2,1):
            for j in range (i+1,length_sitios-1,1):
                for k in range (j+1,length_sitios,1):
                    interseccion= svr_fuzz_Equipartition.triple_intersect(sitios[i],sitios[j],sitios[k])
    ...
    def external_vertices(poligono, sitios):
        ...
        for i in range(0,number_of_regions,1):
            regiones.append(svr_fuzz_Equipartition.puntos_en_region_n(sitios,poligono,i))
    ...
    def particion(poligono, sitios):
        ...
        for i in range(0,number_of_regions,1):
            ...
            a = np.vstack((vert,poligono[svr_fuzz_Equipartition.puntos_en_region_n(sitios,poligono,i)]))
            regiones.append(a)
        ...
        for row, side in enumerate(vor.ridge_vertices):
            if side[0]!=-1:
                ...
                for j in range(-1,np.shape(poligono)[0]-1,1):
                    ...
                    w2x = poligono[j+1][0]
                    w2y = poligono[j+1][1]
                    if svr_fuzz_Equipartition.segment_segment_intersect(v1x,v1y,v2x,v2y,w1x,w1y,w2x,w2y) != None:
                        x , y = svr_fuzz_Equipartition.segment_segment_intersect(v1x,v1y,v2x,v2y,w1x,w1y,w2x,w2y)
                ...
                cercano= svr_fuzz_Equipartition.punto_mas_cercano(sitios_sin,vertex)
                ...
                for j in range(-1,np.shape(poligono)[0]-1,1):
                    ...
                    w2x = poligono[j+1][0]
                    w2y = poligono[j+1][1]
                    if svr_fuzz_Equipartition.segment_line_intersect(w1x,w1y,w2x,w2y,v1x,v1y,v2x,v2y) != None:
                        x , y = svr_fuzz_Equipartition.segment_line_intersect(w1x,w1y,w2x,w2y,v1x,v1y,v2x,v2y)
                        new_intersection = np.array([[x,y]])
                        region_1_to_add = vor.ridge_points[row][0]
                        region_2_to_add = vor.ridge_points[row][1]
                        regiones[region_1_to_add] = np.vstack((regiones[region_1_to_add],new_intersection))
    ...
    def AP(poligono,number_of_regions,external,intermediate,internal):
        ...
        parti=svr_fuzz_Equipartition.particion_vertices(external,intermediate,internal)
    ...
    def Jac_AP(poligono, number_of_regions,external, intermediate, internal, delta):
        ...
        b = svr_fuzz_Equipartition.AP(poligono, number_of_regions,external, intermediate, internal)
        ...
        for column in range(0,n,1):
            ...
            cambio = (svr_fuzz_Equipartition.AP(poligono,number_of_regions,external,intermediate,internal_new)-b)/delta
        ...
        for row in range(p):
            ...
            cambio = (svr_fuzz_Equipartition.AP(poligono,number_of_regions,external,intermediate_new,internal)-b)/delta
            jac = np.append(jac, cambio, axis=1)
        t_intm = time.process_time() - t2
        t_total = time.process_time() - t_start

        # สะสมเวลาลงตัวแปรคลาส
        ct = svr_fuzz_Equipartition.cumulative_jacap_times
        ct['AP_initial']       += t_ap
    ...
    def partition(polygon,number_of_regions,trial_id,round,order,seed):
        ...
        while (alcanzo_resultado==False) and (repetition < numero_de_repetitiones):
            ...
            if (balancear_poligono == True) :
                ...
                nuevo_poligono = svr_fuzz_Equipartition.balanceo(poligono, poligono)
            ...
            part = svr_fuzz_Equipartition.particion(poligono,coordenadas)
            regiones = part[0]
            ...
            for i in range(len(regiones)):
                centros[i]=svr_fuzz_Equipartition.centroide(regiones[i])[0]
            coord = coordenadas
            t1=0
            ...
            part = svr_fuzz_Equipartition.particion(poligono,coordenadas)
            external=svr_fuzz_Equipartition.external_vertices(poligono,coordenadas)
            intermediate=part[3:]
            internal=svr_fuzz_Equipartition.internal_vertices(poligono,coordenadas)
            ...
            if (balancear_poligono == True) :
                internal_new=[]
                internal_new.append(svr_fuzz_Equipartition.desbalanceo(poligono_original, internal[0]))
                internal_new.append(internal[1])
                intermediate_new=[]
                intermediate_new.append(svr_fuzz_Equipartition.desbalanceo(poligono_original, intermediate[0]))
                intermediate_new.append(intermediate[1])
                intermediate_new.append(intermediate[2])
                external_new=[]
                external_new.append(svr_fuzz_Equipartition.desbalanceo(poligono_original, external[0]))
            ...
            while (t < iteraciones_Newton) and ((error_total_normalizado >10**-16)):
                ...
                jacobian = svr_fuzz_Equipartition.Jac_AP(poligono,number_of_regions,external,intermediate,internal,10**-4)
                AP_value = svr_fuzz_Equipartition.AP(poligono,number_of_regions,external,intermediate,internal)
                ...
                parti = svr_fuzz_Equipartition.particion_vertices(external,intermediate_new,internal_new)
                ...
                filled_len = int(percent / 100 * bar_length)
                bar = '0' * filled_len + '-' * (bar_length - filled_len)
                print(f"\r Re : {number_of_regions} Set : {order} eqq : {eqq[3]} Iterations Normal Flow : |{bar}| {percent:.1f}%",end='', flush=True)
            ...
            save_csv = (f'TR : {trial_id}_{round} Total_Time: {endtime} CPU_time : {end_cputime}',
                ...
                ,svr_fuzz_Equipartition.get_jacap_timings())
            sigmaa = 0
            svr_fuzz_Equipartition.save_regions_to_csv(parti[0],os.path.join('k99', f'regions_coordinates_{trial_id}_{round}.csv'),trial_id,
            ...
            stlloyd = time.perf_counter()
            stlloydcpu = time.process_time()
            sigmaa = predict_sigma(polygon, model="svr", models_dir=".")
            for i in range(len(regiones)):
                centros[i]=svr_fuzz_Equipartition.centroide_sigma(regiones[i],sigmaa)[0]
            coord = coordenadas
            t1=0
            while (t1 < iteraciones_centroide) and (np.linalg.norm(coord-centros) >10**-4):
                part = svr_fuzz_Equipartition.particion(poligono,coordenadas)
